...
app.post("/api/switch-db", async (req: Request, res: Response) => {
  ...
  console.log(`Received request to switch database to: ${dbName}`);
  if (!dbName) {
...
app.get("/api/current-db", async (req: Request, res: Response) => {
  try {
    ...
    const currentDbName = result.recordset[0].db_name;
    console.log(`Current database reported by server: ${currentDbName}`);
    res.json({ dbName: currentDbName });
  } catch (err) {
